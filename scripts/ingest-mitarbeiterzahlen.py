"""
ingest-mitarbeiterzahlen.py — Raw Layer Ingestion (File-level): eine CSV-Datei
= ein Iceberg-Row.

raw_payload enthaelt den KOMPLETTEN CSV-Inhalt als UTF-8-String — byte-identisch
zum Original (inkl. Header, Newlines). sha256(raw_payload.encode("utf-8"))
muss immer == source_file_hash sein.

Es findet KEIN strukturiertes CSV-Parsing statt. Die Bytes werden 1:1 als
String gespeichert. Header-Erkennung und Spalten-Extraktion sind Staging-Aufgabe
(siehe dbt/models/staging/stg_mitarbeiterzahlen.sql).

Ausfuehrung im spark-master Container:
  docker compose exec spark-master \\
    spark-submit /scripts/ingest-mitarbeiterzahlen.py \\
      --file /data/sample/mitarbeiterzahlen.csv \\
      --ingestion-timestamp 2026-07-24T09:00:00Z
"""
...
def build_spark() -> SparkSession:
    spark = SparkSession.builder.appName("mitarbeiterzahlen-ingestion").getOrCreate()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest Mitarbeiterzahlen CSV (file-level) into nessie.raw.mitarbeiterzahlen"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="ISO8601",
        help="UTC timestamp for this run, e.g. 2026-07-24T09:00:00Z (default: now)",
    )
    parser.add_argument(
        "--source-version",
    ...
    log(f"Current snapshot count for raw.mitarbeiterzahlen: {snap_count}")
