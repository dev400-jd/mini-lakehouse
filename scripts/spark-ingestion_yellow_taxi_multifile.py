...
# files = [
#     f"{DATA_DIR}/yellow_tripdata_2012-01.parquet",
# ]


table_exists = spark.catalog.tableExists("nessie.raw.yellowtripdata")

for file in files:
    print(f"Lese {file.name}")

    df = spark.read.parquet(str(file))
# df = spark.read.parquet(f"{DATA_DIR}/yellow_tripdata_2012-01.parquet")

# df.writeTo("nessie.raw.yellowtripdata").tableProperty("write.format.default", "parquet").tableProperty("location", f"{RAW_BUCKET}/yellowtripdata").create()
    
    if not table_exists:
        (
            df.writeTo("nessie.raw.yellowtripdata")
        ...
        df.writeTo("nessie.raw.yellowtripdata").append()
