"""
init-cdp-table.py — Droppe + lege nessie.raw.cdp_emissions im File-level Schema neu an.
...
Variante B (eine CSV-Datei = ein Iceberg-Row mit raw_payload als String).
...
RAW_LOCATION = "s3a://raw/cdp_emissions"
...
def main() -> None:
    spark = SparkSession.builder.appName("init-cdp-table").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")
    spark.conf.set("spark.sql.session.timeZone", "UTC")

    spark.sql("CREATE NAMESPACE IF NOT EXISTS nessie.raw")

    existing = spark.sql("SHOW TABLES IN nessie.raw LIKE 'cdp_emissions'").collect()
    if existing:
